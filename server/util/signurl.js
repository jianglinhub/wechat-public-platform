...
var getSignUrl = function(service,reqBody,originalUrl){
    ...
    if(appkey){
        ...
        if(Object.prototype.toString.call(reqBody) === "[object Object]"){
            ...
            var uriObj = signUri(service,originalUrl);
            var sign = signature(uriObj.signUri,params);
            reqBody["sign"] = sign;
            var queryStr = querystring.stringify(reqBody, null, null, { encodeURIComponent: function(a){
                return encodeURIComponent(a);
            }});
            url = uriObj.host + uriObj.signUri + "?" + queryStr;
        }else{
...
var signUri = function(service,originalUrl){
    var result = {};
    if(originalUrl){
        var servicePathArr = service.split("/");
        var serviceUri = config[servicePathArr[0]]["uri"];
        result.host = config[servicePathArr[0]]["host"];
        if(servicePathArr.length > 1){
            for(var i=1;i<servicePathArr.length;i++){
